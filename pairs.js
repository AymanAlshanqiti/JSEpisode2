...
function pairs(names) {
  // To store the randome value's in it

  let randomArray = [];
  // If names not empty and it's length greater than 0
  if (names && names.length > 0) {
    while (names.length > 0) {
      if (names.length !== 1) {
        randomArray.push([names.getRandom(), names.getRandom()]);
      } else {
        randomArray.push([names.getRandom()]);
      }
    }
    return randomArray;

    // If names doesn't have any value or it has an empty array
  } else {
    return [];
  }
}
